...
namespace IceCreamShopView
{
    public partial class FormAdditives : Form
    {
        ...
        public FormAdditives(ILogger<FormAdditives> logger, IAdditiveLogic logic)
        {
            InitializeComponent();
            _logger = logger;
            _logic = logic;
        ...
        {
            var form = DependencyManager.Instance.Resolve<FormAdditive>();
        ...
        private void ButtonDel_Click(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                if (MessageBox.Show("Удалить запись?", "Вопрос", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        if (!_logic.Delete(new AdditiveBindingModel
                        {
                            Id = id
                        }))
                        {
